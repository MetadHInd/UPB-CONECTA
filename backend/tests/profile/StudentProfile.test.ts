...
describe('HU-37 — StudentProfile: frontera solo lectura / editable (criterio 2)', () => {
  it('la proyección del directorio y el semestre no se pueden reasignar (comprobado por tsc)', () => {
    const profile = StudentProfile.fromDirectory(directory(), BOUNDS, T0);
    ...
    const attempts = () => {
      // @ts-expect-error `program` proviene del directorio: solo lectura.
      profile.directory.program = 'medicina';
      ...
      profile.directory = { email: 'x', program: 'y' };
  ...
  it('withSemester es la única vía de cambio y devuelve un perfil nuevo con origen "student"', () => {
    const original = StudentProfile.fromDirectory(directory(), BOUNDS, T0);
  ...
  it('clasifica los cambios pedidos: campos del directorio, editables y desconocidos', () => {
    ...
    expect(classifyProfileChanges({ program: 'medicina', name: 'X', semester: 6, color: 'azul' })).toEqual({
      readOnly: ['program', 'name'],
      unknown: ['color'],
...
describe('HU-37 — sincronización con el directorio', () => {
  it('el primer perfil toma programa y semestre del directorio y normaliza el correo', () => {
    ...
    expect(profile.directory).toEqual({ email: 'estudiante@upb.edu.co', program: 'sistemas' });
  ...
  it('actualiza los campos del directorio y conserva el semestre editado por el estudiante', () => {
    const edited = StudentProfile.fromDirectory(directory(), BOUNDS, T0).withSemester(SemesterNumber.create(8, BOUNDS), T0);

    const synced = edited.syncedWith(directory({ program: 'industrial', semester: 5 }), BOUNDS, T1);

    expect(synced.directory.program).toBe('industrial');
  ...
  it('si el estudiante nunca editó, el semestre sigue al directorio', () => {
    const fromDirectory = StudentProfile.fromDirectory(directory({ semester: 5 }), BOUNDS, T0);

    const synced = fromDirectory.syncedWith(directory({ semester: 6 }), BOUNDS, T1);
  ...
  it('un semestre del directorio fuera de rango no rompe el login: queda desconocido', () => {
    const profile = StudentProfile.fromDirectory(directory({ semester: 0 }), BOUNDS, T0);
  ...
  it('segment() expone solo programa y semestre efectivo', () => {
    const profile = StudentProfile.fromDirectory(directory(), BOUNDS, T0).withSemester(SemesterNumber.create(9, BOUNDS), T1);
...
describe('HU-37 — minimización de datos (criterio 6)', () => {
  it('el perfil no retiene el nombre ni ningún otro dato del directorio que no segmente', () => {
    ...
    const profile = StudentProfile.fromDirectory(record, BOUNDS, T0);

    expect(Object.keys(profile.directory).sort()).toEqual(['email', 'program']);
