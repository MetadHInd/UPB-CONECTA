...
`src/contexts/notifications/` es el segundo contexto fuera de `ingestion` — mismo patrón hexagonal, ver su propio README. A medida que se implementen más historias, se añadirán más carpetas bajo `src/contexts/` (por ejemplo `classification`, `feed`, `moderation`).
...
npm test                     # 132 pruebas (requiere MongoDB corriendo)
