...
describe('IngestionScheduler, criterios de aceptacion 1 y 2', () => {
  ...
  it('dispara el caso de uso en el intervalo configurado sin intervencion humana', async () => {
    ...
    const scheduler = new IngestionScheduler(spy.port, () => ({ intervalMs: 60_000, batchSize: 200 }));
  ...
  it('aplica un cambio de intervalo sin reiniciar el proceso', async () => {
    ...
    const scheduler = new IngestionScheduler(spy.port, () => ({ intervalMs, batchSize: 200 }));
  ...
  it('no detiene el servicio cuando un ciclo falla', async () => {
    ...
    const scheduler = new IngestionScheduler(port, () => ({ intervalMs: 30_000, batchSize: 200 }), (e) => errores.push(e));
    scheduler.start();
  ...
  it('no solapa ciclos cuando una ejecucion tarda mas que el intervalo', async () => {
    ...
    const scheduler = new IngestionScheduler(port, () => ({ intervalMs: 30_000, batchSize: 200 }));
    scheduler.start();
  ...
  it('es seguro llamar start dos veces y stop sin haber arrancado', () => {
    const spy = useCaseSpy();
    const scheduler = new IngestionScheduler(spy.port, () => ({ intervalMs: 30_000, batchSize: 200 }));
    scheduler.stop();
    scheduler.start();
    scheduler.start();
